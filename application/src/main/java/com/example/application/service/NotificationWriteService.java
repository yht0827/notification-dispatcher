...
public class NotificationWriteService implements NotificationWriteUseCase {
	...
	public boolean markAsRead(Long notificationId) {
		var from = NotificationDetailRetentionPolicy.detailFrom(java.time.LocalDateTime.now());
		return notificationRepository.findById(notificationId)
			.filter(
				notification -> NotificationDetailRetentionPolicy.isWithinRetention(notification.getCreatedAt(), from))
			.map(notification -> {
				notificationReadStatusRepository.markAsRead(notificationId, java.time.LocalDateTime.now());
				return true;
			})
			.orElse(false);
	}
